...
public interface TransactionRepository extends CrudRepository<Transaction, Long>, PagingAndSortingRepository<Transaction, Long> {
	...
	@Query("Select t from Transaction t where DATE_FORMAT(t.revenueDepartmentDate,'%Y-%m') = :revenueMonth and t.holdingTax = true and t.transactionStatus='normal'")
	public Page<Transaction> findAllHoldingTaxByMonth(@Param("revenueMonth") String revenueMonth, Pageable pageable);
